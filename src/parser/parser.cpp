...
static void show_usage() {
  ...
  std::cerr << "Usage:\n bashDB [ACTION] [OPTIONS]...\n"
            ...
            << "\tset [k] [v]\t\tSet the corresponding [value] for [key] (will "
               "overwrite)\n"
            << "\tsafeset [k] [v]\t\tSet the corresponding [value] for [key] "
               "if the key does not already exist (prevents overwriting)\n"
            << "Options:\n"
            << "\t-h,--help\t\tShow this help message\n"
            << "\t-k,--key\t\tThe key utilized for the [action]. Required for "
               "[get], [set], [safeset]\n"
            << "\t-v,--value\t\tThe value to assign for the [key]. Required "
               "for [set], [safeset]"
            << std::endl;
}
